...
describe('the payment receipt email', () => {
  ...
  it('ships with a link that opens the review box itself', () => {
    goLive();
    const id = invoiced(500);
    const result = pay(id, {
  ...
  it('tells the customer where to look when the link only opens a page', () => {
    goLive();
    // Still reachable: somebody pastes a listing URL over the default. The
    // ask is worse, but saying nothing about it would be worse again.
    backend.fn('setReviewUrl', adminToken, 'https://www.google.com/search?kgmid=/g/1thkxvf7');
  ...
  it('refuses a review link that is not a real https link', () => {
    ...
    // And the refusal changes nothing — the built-in link is still in force.
    expect(backend.fn('config', adminToken).reviewUrl).toContain('writereview');
    expect(backend.fn('config', adminToken).reviewUrlDirect).toBe(true);
  });
